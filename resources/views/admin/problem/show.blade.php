...
@section('content')
        ...
        <div class="card">
            ...
            <div class="p-5 border-b flex justify-between items-center">
                ...
                <div class="flex space-x-2">
                    <a href="{{ route('solution.create') }}?problem_id={{ $problem->id }}"
                        class="btn-shadow bg-blue-400">Add
                        Solution</a>
            ...
            <div class="card-body grid grid-cols-3 gap-6 lg:grid-cols-1">
                ...
                <div class="">
                    <div class="grid gap-2 grid-flow-row grid-cols-2 coderDairy__problem_media_image">
                        {{-- {{ dd($problem) }} --}}
                        @foreach ($problem->media as $media)
                            <a href="{{ $media->name['url'] }}">
                                <img src="{{ $media->name['url'] }}" class="m-1 rounded-md" alt="">
                            </a>
                        @endforeach
                    </div>
                </div>
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-1 mt-3">
        <div class="card">
            <!-- body -->
            ...
            <div class="">
                ...
                @forelse ($solutions as $index=>$solution)
                    ...
                    <div class="coderDairy__accrodion">
                        <div class="accordion_content">
                            <div class="label rounded bg-gray-500">Solution #{{ $index + 1 }}</div>
                            <div class="content flex">
                                <div class="flex-1">{!! $solution->content !!}</div>
                                <div class="flex-1">
                                    <div class="coderDairy__problem_media_image justify-end">

                                        @foreach ($solution->media as $media)
                                            <a href="{{ $media->name['url'] }}" data-lightbox="mygallery"
                                                data-title="My caption"><img class="rounded-md"
                                                    src="{{ $media->name['url'] }}" alt=""></a>
                                        @endforeach
                ...
                @empty
                    <div class="text-center p-4">
                        <h3 class="text-lg text-red-600">No Solution found.</h3>
                    </div>
                @endforelse
...
@section('scripts')
    <script>
        $(document).ready(function() {
            $('.coderDairy__problem_media_image').magnificPopup({
                delegate: 'a',
                type: 'image',
                gallery: {
                    enabled: true
